        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Senarai Kampus</li>
                </ol>
            </nav>
        </div>
        <div class="ms-auto">
        ...
        <div class="card-body">
            <div class="d-lg-flex align-items-center mb-4 gap-3">
                <div class="position-relative">
                    <form action="{{ route('campus.search') }}" method="GET" id="searchForm"
                    ...
                    <tbody>
                        @if (count($campusList) > 0)
                            @foreach ($campusList as $campus)
                                <tr>
                                    <td>
                                        {{ ($campusList->currentPage() - 1) * $campusList->perPage() + $loop->iteration }}
                                    </td>
                                    <td>{{ $campus->name }}</td>
                                    <td>
            ...
            <div class="mt-3 d-flex justify-content-between">
                <div class="d-flex align-items-center">
                    ...
                    <form action="{{ route('campus.search') }}" method="GET" id="perPageForm"
                ...
                <div class="d-flex justify-content-end align-items-center">
                    <span class="mx-2 mt-2 small text-muted">
                        Menunjukkan {{ $campusList->firstItem() }} hingga {{ $campusList->lastItem() }} daripada
                        {{ $campusList->total() }} rekod
                    </span>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        @isset($campus)
                            Adakah anda pasti ingin memadam rekod <span style="font-weight: 600;">Kampus
                                {{ $campus->name }}</span>?
                        @else
                    ...
                    <div class="modal-footer">
                        ...
                        @isset($campus)
                            <form class="d-inline" method="POST" action="{{ route('campus.destroy', $campus->id) }}">
                                {{ method_field('delete') }}
                                {{ csrf_field() }}
