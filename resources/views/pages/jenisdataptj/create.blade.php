            ...
            <form method="POST" action="{{ $save_route }}">
                ...
                <div class="mb-3">
                    ...
                    @if ($errors->has('subunit'))
                        <div class="invalid-feedback">
                            @foreach ($errors->get('subunit') as $error)
                                {{ $error }}
                            @endforeach
                        </div>
                    @endif
                ...
                if (departmentId) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.length === 0) {
                                $('#subunit_id').append(
                                    '<option disabled selected>Tiada Sub Unit</option>');
                            } else {
                                $('#subunit_id').append(
                                    '<option disabled selected>Pilih Sub Unit</option>');
                                $.each(data, function(index, subunit) {
                                    $('#subunit_id').append('<option value="' + subunit
                                        .id + '">' + subunit.name + '</option>');
                                });
                            }
                        },
                        error: function(xhr) {
                            alert("Ralat ambil sub unit.");
                            console.log(xhr.responseText);
                        }
                    });
                } else {
                    $('#subunit_id').empty().append('<option disabled selected>Pilih Sub Unit</option>');
                }
            });
        });
    </script>
