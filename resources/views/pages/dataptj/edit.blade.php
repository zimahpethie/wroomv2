...
@section('content')
        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ $str_mode }} Data PTJ</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <h6 class="mb-0 text-uppercase">{{ $str_mode }} Data PTJ</h6>
    <hr />
        ...
        <div class="card-body">
            <form method="POST" action="{{ $save_route }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="{{ $dataptj->department->name ?? '-' }}" readonly>
                    <input type="hidden" name="department_id" value="{{ $dataptj->department_id }}">
                </div>
                        ...
                        <select class="form-select {{ $errors->has('subunit_id') ? 'is-invalid' : '' }}" name="subunit_id">
                            ...
                            @foreach ($subunitList as $unit)
                                <option value="{{ $unit->id }}"
                                    {{ old('subunit_id', $dataptj->subunit_id) == $unit->id ? 'selected' : '' }}>
                                    {{ $unit->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                {{-- Tajuk Data --}}
                <div class="mb-3">
                    <label for="nama_data" class="form-label">Tajuk Data</label>
                    <input type="text" class="form-control {{ $errors->has('nama_data') ? 'is-invalid' : '' }}"
                        name="nama_data" value="{{ old('nama_data', $dataptj->nama_data ?? '') }}" required>
                    @if ($errors->has('nama_data'))
                        <div class="invalid-feedback">
                            {{ $errors->first('nama_data') }}
                        </div>
                    @endif
                </div>
                ...
                <div class="mb-3">
                    ...
                    @php
                        // fallback: old() > $dataptj->jenis_nilai > null
                        $selectedJenisNilai = old('jenis_nilai', $dataptj->jenis_nilai ?? null);
                    @endphp
                ...
                <div class="mb-3">
                    ...
                    <input type="url" class="form-control {{ $errors->has('doc_link') ? 'is-invalid' : '' }}"
                        name="doc_link" value="{{ old('doc_link', $dataptj->doc_link) }}">
