    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ $save_route }}">
                ...
                {{-- Jenis Data --}}
                <div class="mb-3">
                    <label for="jenis_data_ptj" class="form-label">Tajuk Data</label>
                    <input type="text" name="jenis_data_ptj"
                        class="form-control {{ $errors->has('jenis_data_ptj') ? 'is-invalid' : '' }}"
                        value="{{ old('jenis_data_ptj') }}" required>
                    @if ($errors->has('jenis_data_ptj'))
                        <div class="invalid-feedback">
                            @foreach ($errors->get('jenis_data_ptj') as $error)
                                {{ $error }}
                            @endforeach
                        </div>
                    @endif
